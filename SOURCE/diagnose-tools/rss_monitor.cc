...
void usage_rss_monitor(void)
{
	...
	//printf("          user-symbol set user symbol(default: 1)\n");
	printf("        --log \n");
...
static int rss_monitor_extract(void *buf, unsigned int len, void *)
{
	...
	case et_rss_monitor_detail:
		...
		seq++;
		printf("##CGROUP:[%s]  %d      [%03d]  命中，时间：[%lu:%lu]\n",
...
static int sls_extract(void *buf, unsigned int len, void *)
{
	...
	case et_rss_monitor_detail:
		...
		write_file(sls_file, "rss-monitor-detail", &detail->tv, 0, 0, root);
		write_syslog(syslog_enabled, "rss-monitor-detail", &detail->tv, 0, 0, root);
		break;
	case et_rss_monitor_raw_detail:
		if (len < sizeof(struct rss_monitor_raw_stack_detail))
			break;
		...
		write_file(sls_file, "rss-monitor-raw-detail", &raw_detail->tv, 0, 0, root);
		write_syslog(syslog_enabled, "rss-monitor-raw-detail", &raw_detail->tv, 0, 0, root);
		break;
	default:
		break;
	}
