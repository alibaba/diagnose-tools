...
static int run_trace_extract_2(void *buf, unsigned int len, void *)
{
	...
	{
		struct event_sys_enter_raw *event = (struct event_sys_enter *)buf;
		...
		for (i = 0; i < BACKTRACE_DEPTH; i++) {
			if (event->raw_stack.stack[i] == (size_t)-1 || event->raw_stack.stack[i] == 0) {
				break;
			}
			sym.reset(event->raw_stack.stack[i]);
